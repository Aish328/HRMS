...
dashboardRouter.get('/me', (req, res) => {
  ...
  const pendingLeaves = db.prepare(
    "SELECT COUNT(*) c FROM leaves WHERE user_id = ? AND status IN ('pending','pending_hr','changes_requested')"
  ).get(req.user.id).c;

  const upcomingHolidays = db.prepare(
    'SELECT date, name FROM holidays WHERE date >= ? ORDER BY date LIMIT 4'
  ).all(today);

  const monthRecords = db.prepare(
    'SELECT working_minutes, punch_out_at, work_date FROM attendance WHERE user_id = ? AND work_date >= ?'
  ).all(req.user.id, monthStart);

  res.json({ pendingLeaves, upcomingHolidays, monthDaysPresent: monthRecords.length });
});

// ---- Manager team dashboard (any employee who has direct reports) ----
dashboardRouter.get('/manager', (req, res) => {
  const today = todayStr();
  const reports = db.prepare(
    `SELECT u.id, u.name, u.employee_code, u.designation,
       a.punch_in_at, a.punch_out_at, a.working_minutes
     FROM users u
     LEFT JOIN attendance a ON a.user_id = u.id AND a.work_date = ?
     WHERE u.manager_id = ? AND u.status = 'active' ORDER BY u.name`
  ).all(today, req.user.id);

  const pendingApprovals = db.prepare(
    `SELECT COUNT(*) c FROM leaves l JOIN users u ON u.id = l.user_id
     WHERE u.manager_id = ? AND l.status = 'pending'`
  ).get(req.user.id).c;

  const onLeave = db.prepare(
    `SELECT u.name FROM leaves l JOIN users u ON u.id = l.user_id
     WHERE u.manager_id = ? AND l.status = 'approved' AND l.start_date <= ? AND l.end_date >= ?`
  ).all(req.user.id, today, today);

  res.json({
    isManager: reports.length > 0,
    team: reports,
    presentToday: reports.filter((r) => r.punch_in_at).length,
    pendingApprovals,
    onLeave,
  });
});

dashboardRouter.get('/summary', requireRole('admin'), (_req, res) => {
  const today = todayStr();
  ...
  const pendingLeaves = db.prepare("SELECT COUNT(*) c FROM leaves WHERE status IN ('pending','pending_hr')").get().c;
