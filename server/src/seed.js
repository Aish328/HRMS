...
db.exec('DELETE FROM leave_approvals; DELETE FROM holidays; DELETE FROM notifications; DELETE FROM activity_log; DELETE FROM leaves; DELETE FROM attendance; DELETE FROM users; DELETE FROM departments;');
...
const adminId = Number(insUser.run('ADM001', 'Priya Sharma', 'admin@company.com', hash('admin123'), 'admin', deptIds['Human Resources'], 'HR Manager', '+91 98100 00001', '2021-04-01').lastInsertRowid);
...
insLeave.run(empIds[1], 'sick', past(6), past(5), 2, 'Viral fever, doctor advised rest.', 'approved', adminId, past(6));
insLeave.run(empIds[3], 'earned', future(10), future(14), 5, 'Planned vacation with family.', 'pending', null, null);
insLeave.run(empIds[4], 'casual', past(12), past(12), 1, 'Personal errand.', 'rejected', adminId, past(12));
insLeave.run(empIds[6], 'sick', todayStr(), future(1), 2, 'Migraine, unable to travel.', 'approved', adminId, past(1));
...
// Set project + company on all users
db.exec("UPDATE users SET company = 'Sharika Enterprises Limited'");
const setPC = db.prepare('UPDATE users SET project = ?, designation = ? WHERE id = ?');
setPC.run('Corporate', 'Managing Director', adminId);
setPC.run('SCADA Modernisation', 'Project Manager', empIds[0]);   // Arjun
setPC.run('SCADA Modernisation', 'SCADA Engineer', empIds[1]);    // Sneha
setPC.run('Smart Metering', 'Project Manager', empIds[2]);        // Rahul
setPC.run('Smart Metering', 'Field Engineer', empIds[3]);         // Ananya
setPC.run('Corporate', 'Chief Financial Officer', empIds[4]);     // Vikram
setPC.run('Smart Metering', 'Data Analyst', empIds[5]);           // Kavya
setPC.run('SCADA Modernisation', 'Automation Engineer', empIds[6]); // Rohan
setPC.run('Smart Metering', 'QA Engineer', empIds[7]);            // Meera

// Reporting hierarchy:
//   Vikram (Sales Manager) → manages Ananya
//   Rahul (Operations Lead) → manages Kavya, Meera
//   Arjun (Senior Developer) → manages Sneha, Rohan
const setMgr = db.prepare('UPDATE users SET manager_id = ? WHERE id = ?');
// Single-root hierarchy: MD -> {CFO, PM Arjun, PM Rahul} -> team members
setMgr.run(adminId, empIds[4]);   // Vikram (CFO) → MD
setMgr.run(adminId, empIds[0]);   // Arjun (PM) → MD
setMgr.run(adminId, empIds[2]);   // Rahul (PM) → MD
setMgr.run(empIds[0], empIds[1]); // Sneha → Arjun
setMgr.run(empIds[0], empIds[6]); // Rohan → Arjun
setMgr.run(empIds[2], empIds[5]); // Kavya → Rahul
setMgr.run(empIds[2], empIds[7]); // Meera → Rahul
setMgr.run(empIds[2], empIds[3]); // Ananya → Rahul

// Holidays (India, sample — admins can add more from the API)
const insHol = db.prepare('INSERT OR IGNORE INTO holidays (date, name) VALUES (?,?)');
insHol.run('2026-08-15', 'Independence Day');
insHol.run('2026-10-02', 'Gandhi Jayanti');
insHol.run('2026-10-20', 'Diwali');
insHol.run('2026-12-25', 'Christmas');
insHol.run('2027-01-26', 'Republic Day');
